...
export class UploadsService {
    ...
    async saveAvatar(file: Express.Multer.File): Promise<string> {
        ...
        if (this.s3Client && this.bucketName) {
            ...
            try {
                ...
                console.log(`S3 PutObject successful for key: ${key}`);
                // Return proxy URL
                return `/uploads/file/${key}`;
            } catch (error) {
                console.error('Error saving avatar to S3:', error);
                throw error;
            }
        } else {
    ...
    async saveAttachment(file: Express.Multer.File, pathSegments: string[] = []): Promise<{ url: string; type: string; filename: string }> {
        ...
        if (this.s3Client && this.bucketName) {
            ...
            try {
                ...
                console.error('Error saving attachment to S3:', error);
                throw error;
            }
        } else {
    ...
    async getFileStream(key: string): Promise<{ stream: Readable; contentType: string }> {
        if (this.s3Client && this.bucketName) {
            try {
                console.log(`[DEBUG] getFileStream requesting key: "${key}"`);
                ...
                // DEBUG: List objects to see what's actually there
                try {
                    // Extract prefix (e.g., "avatars/" or "attachments/")
                    const prefix = key.split('/')[0] + '/';
                    console.log(`[DEBUG] Listing objects in bucket ${this.bucketName} with prefix "${prefix}" to debug...`);
                    ...
                    console.log('[DEBUG] Found objects:', listRes.Contents?.map(o => `"${o.Key}"`));
                } catch (listErr) {
                    console.error('[DEBUG] Failed to list objects:', listErr);
                }
